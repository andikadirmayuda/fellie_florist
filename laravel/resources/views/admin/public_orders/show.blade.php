<x-app-layout>
    ...
    <div class="container mx-auto py-8">
        <div class="mb-4">
            <a href="{{ route('admin.public-orders.index') }}" class="text-blue-600 hover:underline">&larr; Kembali ke
                daftar pesanan</a>
        </div>
            ...
            <div class="mb-4">
                ...
                <div class="flex w-full justify-between mb-2">
                    @foreach($steps as $key => $label)
                        <div class="flex-1 flex flex-col items-center">
                            <div
                                class="rounded-full w-8 h-8 flex items-center justify-center mb-1 {{ $currentStatus === $key || $currentIndex > array_search($key, $stepKeys) ? 'bg-pink-600 text-white' : 'bg-gray-300 text-gray-500' }}">
                                {{ $loop->iteration }}
                            </div>
                            <div
                                class="text-xs text-center {{ $currentStatus === $key || $currentIndex > array_search($key, $stepKeys) ? 'text-pink-600 font-bold' : 'text-gray-500' }}">
                                {{ $label }}</div>
                        </div>
                        @if(!$loop->last)
                            <div
                                class="flex-1 h-1 bg-gray-300 mx-1 sm:mx-2 mt-3 {{ $currentIndex >= $loop->index ? 'bg-pink-600' : '' }}">
                            </div>
                        @endif
                    @endforeach
                </div>
                ...
                @if(!in_array($order->status, ['completed', 'done', 'cancelled', 'canceled']))
                    <form method="POST" action="{{ route('admin.public-orders.update-status', $order->id) }}"
                        class="inline-flex items-center gap-2 mt-2" enctype="multipart/form-data" id="statusForm">
                        @csrf
        ...
        <div class="bg-white rounded shadow p-6 mb-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                @endphp
                <span class="inline-block px-2 py-1 rounded text-white font-semibold text-sm"
                    style="background: {{ $paymentBg }}; min-width:150px; text-align:center;">
                    ...
                    <form method="POST" action="{{ route('admin.public-orders.update-payment-status', $order->id) }}"
                        ...
                        <select name="payment_status" class="border rounded p-1 mx-2" id="paymentStatusSelect">
                            @foreach($paymentStatusMap as $key => $label)
                                <option value="{{ $key }}" @if($order->payment_status == $key) selected @endif>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @foreach($order->items as $item)
                        <tr>
                            ...
                            <td class="px-4 py-2 border">
                                Rp{{ number_format(($item->price ?? 0) * ($item->quantity ?? 0), 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        @if(!empty($order->payment_proof))
            <div class="bg-white rounded shadow p-6 mb-6">
                ...
                @endif
                <div id="payment-proof-error" style="display:none; color:red;">Bukti pembayaran tidak ditemukan di server.
                </div>
            </div>
        @endif
        ...
        <div class="bg-white rounded shadow p-6 mb-6 text-center">
            @if(!empty($order->public_code))
                <a href="{{ route('public.order.invoice', ['public_code' => $order->public_code]) }}" target="_blank"
                    class="inline-block bg-pink-600 hover:bg-pink-700 text-white font-bold py-2 px-4 rounded mb-2">
                ...
                @if(config('public_order.enable_public_order_edit') && $order->status === 'pending')
                    <a href="{{ route('public.order.edit', ['public_code' => $order->public_code]) }}" target="_blank"
                        class="inline-block bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded mb-2">
                        <i class="bi bi-pencil mr-1"></i>Edit Pesanan (Publik)
                    </a>
                @endif
            @else
                <div class="text-red-600 font-semibold">Kode invoice publik belum tersedia. Silakan edit/migrasi data order
                    ini.</div>
            @endif
        </div>
    </div>
