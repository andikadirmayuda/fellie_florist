...
<div class="max-w-xl mx-auto p-4 bg-white rounded shadow">
    <h2 class="text-lg font-bold mb-4">Master Ukuran Bouquet</h2>
    ...
    <table class="w-full text-sm mt-4">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="p-1">{{ $sz->name }}</td>
                <td class="p-1">
                    <button wire:click="edit({{ $sz->id }})" class="text-blue-600">Edit</button>
                    <button wire:click="delete({{ $sz->id }})" class="text-red-600 ml-2" onclick="return confirm('Hapus ukuran ini?')">Hapus</button>
                </td>
            </tr>
            @endforeach
        </tbody>
