<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('bouquets.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                Kembali
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-gray-900">{{ $bouquet->description ?: 'Tidak ada deskripsi' }}</p>
                                </div>
                            </div>
                        </div>
                        <div>
                            ...
                            @if($bouquet->image)
                                <img src="{{ asset('storage/' . $bouquet->image) }}" alt="{{ $bouquet->name }}" class="rounded-lg shadow-md max-w-full h-auto">
                            @else
                                        ...
                                        <tfoot class="bg-gray-50">
                                            <tr>
                                                <td colspan="2" class="px-6 py-4 text-right">
                                                    ...
                                                    <span class="ml-2">Rp {{ number_format($sizeData['total_cost'], 0, ',', '.') }}</span>
                                                    <br>
                                                    <span class="font-medium">Harga Jual:</span>
                                                    <span class="ml-2">Rp {{ number_format($sizeData['price'], 0, ',', '.') }}</span>
                                                    <br>
                                                    <span class="font-medium">Margin:</span>
                                                    <span class="ml-2 {{ ($sizeData['price'] - $sizeData['total_cost']) > 0 ? 'text-green-600' : 'text-red-600' }}">
                                                        Rp {{ number_format($sizeData['price'] - $sizeData['total_cost'], 0, ',', '.') }}
                                                    </span>
                                                </td>
                                            </tr>
                                        </tfoot>
                    ...
                    <div class="mt-8 flex justify-end space-x-3">
                        <a href="{{ route('bouquet-components.create', ['bouquet' => $bouquet->id]) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            Tambah Komponen
                        </a>
                        <a href="{{ route('bouquets.edit', $bouquet) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                            Edit Buket
                        </a>
                    </div>
                </div>
