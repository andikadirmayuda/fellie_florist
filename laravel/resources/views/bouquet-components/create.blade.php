                ...
                <div class="p-6">
                    <form action="{{ route('bouquet-components.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="bouquet_id" id="bouquet_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                ...
                                @foreach($bouquets as $bouquet)
                                    <option value="{{ $bouquet->id }}" {{ old('bouquet_id') == $bouquet->id ? 'selected' : '' }}>
                                        {{ $bouquet->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="quantity" id="quantity" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('quantity') }}" required min="1">
                            @error('quantity')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            @enderror
                        </div>
