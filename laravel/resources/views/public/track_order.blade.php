...
<body class="bg-gray-100 min-h-screen">
    <div class="container mx-auto px-2 sm:px-4 py-8 min-h-screen">
        <div class="bg-white rounded-lg shadow-lg p-4 sm:p-8 w-full max-w-2xl mx-auto">
            ...
            <form method="GET" action="{{ route('public.order.track') }}" class="flex flex-col sm:flex-row items-center gap-2 mb-6">
                <input type="text" name="wa_number" class="border rounded px-3 py-2 text-sm flex-1" placeholder="Masukkan No. WhatsApp" value="{{ $wa_number ?? '' }}" required>
            ...
            @endif
            @if($orders->count())
                <div class="mb-2 text-gray-700 text-sm">Menampilkan {{ $orders->count() }} pesanan untuk: <b>{{ $wa_number }}</b></div>
                    ...
                    <table class="w-full text-xs sm:text-base border">
                        ...
                        <tbody>
                            ...
                            <tr class="border-t">
                                ...
                                <td class="py-2 px-2">
                                    <a href="{{ route('public.order.detail', ['public_code' => $order->public_code]) }}" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-xs">Lihat Detail</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
