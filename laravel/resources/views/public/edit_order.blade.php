...
<div class="container mx-auto py-8">
    ...
    <form method="POST" action="{{ route('public.order.update', ['public_code' => $order->public_code]) }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="customer_name" class="border rounded w-full p-2" value="{{ old('customer_name', $order->customer_name) }}" required>
        </div>
        <div class="mb-4">
            <label class="block mb-1">Tanggal Ambil/Kirim</label>
            <input type="date" name="pickup_date" class="border rounded w-full p-2" value="{{ old('pickup_date', $order->pickup_date) }}" required>
        </div>
        <div class="mb-4">
            <label class="block mb-1">Waktu Ambil/Pengiriman</label>
            <input type="text" name="pickup_time" class="border rounded w-full p-2" value="{{ old('pickup_time', $order->pickup_time) }}">
        </div>
        <div class="mb-4">
            <label class="block mb-1">Metode Pengiriman</label>
            <input type="text" name="delivery_method" class="border rounded w-full p-2" value="{{ old('delivery_method', $order->delivery_method) }}" required>
        </div>
        <div class="mb-4">
            <label class="block mb-1">Tujuan Pengiriman</label>
            <input type="text" name="destination" class="border rounded w-full p-2" value="{{ old('destination', $order->destination) }}">
        </div>
        <div class="mb-4">
            <label class="block mb-1">No. WhatsApp</label>
            <input type="text" name="wa_number" class="border rounded w-full p-2" value="{{ old('wa_number', $order->wa_number) }}">
        </div>
            ...
            @foreach($order->items as $i => $item)
                <tr class="item-row">
                    <td class="border px-2 py-1">
                        ...
                        <select name="items[{{ $i }}][product_id]" class="border rounded p-1 w-full product-select" required data-row="{{ $i }}">
                            <option value="">Pilih Produk</option>
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" data-prices='@json($product->prices)' {{ $item->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td class="border px-2 py-1">
                        <select name="items[{{ $i }}][price_type]" class="border rounded p-1 w-full price-type-select" required data-row="{{ $i }}">
                            <option value="">Pilih Tipe Harga</option>
                            @if($item->product && $item->product->prices)
                                @foreach($item->product->prices as $price)
                                    <option value="{{ $price->type }}" data-price="{{ $price->price }}" data-unit="{{ $price->unit_equivalent }}" {{ $item->price_type == $price->type ? 'selected' : '' }}>{{ $price->type }}</option>
                                @endforeach
                            @endif
                        </select>
                    </td>
                    <td class="border px-2 py-1">
                        <input type="number" name="items[{{ $i }}][price]" class="border rounded p-1 w-full price-input" placeholder="Harga" value="{{ old('items.'.$i.'.price', $item->price) }}" min="0" readonly>
                    </td>
                    <td class="border px-2 py-1">
                        <input type="number" name="items[{{ $i }}][unit_equivalent]" class="border rounded p-1 w-full unit-input" placeholder="Satuan" value="{{ old('items.'.$i.'.unit_equivalent', $item->unit_equivalent) }}" min="1" readonly>
                    </td>
                    <td class="border px-2 py-1">
                        <input type="number" name="items[{{ $i }}][quantity]" class="border rounded p-1 w-full" placeholder="Jumlah" value="{{ old('items.'.$i.'.quantity', $item->quantity) }}" min="1">
                    </td>
        ...
        <button type="button" id="add-item" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded mb-4">+ Tambah Produk</button>

        <button type="submit" class="bg-pink-600 hover:bg-pink-700 text-white font-bold py-2 px-4 rounded">Simpan Perubahan</button>
        <a href="{{ route('public.order.invoice', ['public_code' => $order->public_code]) }}" class="ml-2 text-blue-600 hover:underline">Batal</a>
    </form>
</div>
...
document.getElementById('items-list').addEventListener('change', function(e) {
    ...
    if (e.target.classList.contains('product-select')) {
        ...
        prices.forEach(function(price) {
            priceTypeSelect.innerHTML += `<option value="${price.type}" data-price="${price.price}" data-unit="${price.unit_equivalent}">${price.type}</option>`;
        });
    }
