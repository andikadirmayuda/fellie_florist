<x-app-layout>
    <div class="container mx-auto py-8">
        <h1 class="text-2xl font-bold mb-4 text-pink-600">Pemesanan Buket</h1>
        <div class="bg-white rounded shadow p-6">
            ...
            @endif
            <form method="POST" action="{{ route('bouquet.orders.store') }}">
                @csrf
                <div class="mb-2">
                ...
                <div class="mb-2">
                    ...
                    <input type="text" name="wa_number" class="border rounded px-2 py-1 w-full" required>
                </div>
                <div class="mb-2">
                    <label>Catatan</label>
                ...
                <button type="submit" class="bg-pink-600 hover:bg-pink-700 text-white font-bold px-4 py-2 rounded mt-2">Pesan Buket</button>
            </form>
            <div id="select-flower-template" class="hidden">
                <select class="border rounded px-2 py-1 flower-select-template" required>
                    <option value="">Pilih Bunga</option>
                    @foreach($products as $product)
                        <option value="{{ $product->id }}">{{ $product->name }} (Stok: {{ $product->stock }})</option>
                    @endforeach
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelector('.add-flower').addEventListener('click', function() {
                ...
                // Ambil select dari template
                const selectTemplate = document.querySelector('.flower-select-template').outerHTML.replace('<select', `<select name=\"flowers[${flowerIndex}][product_id]\"`);
                row.innerHTML = `
                    ${selectTemplate}
